...
class SimpleTweenVector2
{
    ...
    update(deltaTime)
    {
        ...
        { 
            ...
            this.valOld = this.val;
            this.val = this.endValue;      
            this.updateCallBack.call(this.contextObject, this.val, MkMath.subtract_V2(this.val, this.valOld));
            this.completeCallBack.call(this.contextObject);
            this.complete = true;
        }
        else
        {
            this.valOld = this.val;
            this.val = MkMath.lerp_V2(this.startValue, this.endValue, this.dTime/this.time);    
            this.updateCallBack.call(this.contextObject, this.val, MkMath.subtract_V2(this.val, this.valOld));
        }
    }
}
...
class SoundController
{
    constructor(scene)
    {
        ...
        this.soundKey = 'mk_sound_key';
        this.musicKey = 'mk_music_key';
        if(localStorage.hasOwnProperty(this.soundKey)) this._soundOn = (parseInt(localStorage.getItem(this.soundKey)) == 1); 
        if(localStorage.hasOwnProperty(this.musicKey)) this._musicOn = (parseInt(localStorage.getItem(this.musicKey)) == 1); 
    }
    ...
    soundOn(soundOn)
    {
        ...
        localStorage.setItem(this.soundKey, this._soundOn ? 1 : 0); 
    }
    ...
    musicOn(musicOn)
    {
        ...
        this._refreshSoundSources();
        localStorage.setItem(this.musicKey, this._musicOn ? 1 : 0); 
    }
...
class PopUpController
{
    ...
    setDepth(depth){
        this.depth = depth;
        this.wCont.depth = depth;
        return this;
    }
}

var MkMath = {
    sum_V2 : function (vector_1, vector_2){
        return new Phaser.Math.Vector2(vector_1.x + vector_2.x, vector_1.y + vector_2.y);;
    },

    subtract_V2 : function (vector_1, vector_2){
        return new Phaser.Math.Vector2(vector_1.x - vector_2.x, vector_1.y - vector_2.y);
    },

    multiply_V2 : function (vector_1, vector_2){
        return new Phaser.Math.Vector2(vector_1.x * vector_2.x, vector_1.y * vector_2.y);
    },

    lerp_V2 : function (vector_1, vector_2, t){

        if (t === undefined) { t = 0; }
        var ax = vector_1.x;
        var ay = vector_1.y;
        var x = ax + t * (vector_2.x - ax);
        var y = ay + t * (vector_2.y - ay);
        return new Phaser.Math.Vector2(x, y);
    },

    float_lerp : function (val1, val2, amount)
    {
        amount = amount < 0.0 ? 0.0 : amount;
        amount = amount > 1.0 ? 1.0 : amount;
        return val1 + (val2 - val1) * amount;
